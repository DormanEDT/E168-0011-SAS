...
uint8_t gu8CSASsendsasoutput[8]={0x00,0x01,0x98,0x00,0xF0,0x00,0x00,0xB6};
uint8_t gu8CSASsendsasoutput2[8]={0x00,0x01,0x00,0x00,0x78,0x78,0x78,0x43};
...
case  CSAS525ABSARBID_SCAN_REQ:
	...
	decrement_flag=1;
     if ((can_msg[0] == 0x00) && (can_msg[1] == 0x00)  && (can_msg[2] == 0x07)  && (can_msg[3] == 0x2B))
	 {
    	 //Cal_Stop=1;
		 CAN_Transmit_msg(&gCANMailBoxNo, CSAS525SASARBID_SCAN_TRX, CAN_ID_STD,
						      CSAS525_SIZE8BYTE,gu8CSASsendsasoutput3);
	 }
     //Test mode
	 else if (((can_msg[0] & 0x08)==0x08) && (can_msg[1] == 0x00)  && (can_msg[2] == 0x07)  && (can_msg[3] == 0x47 ||can_msg[3] == 0x57))
	 {

		 			gu8CSASsendsasoutput3[2]=0x98;
		 			CAN_Transmit_msg(&gCANMailBoxNo, CSAS525SASARBID_SCAN_TRX, CAN_ID_STD,
						      CSAS525_SIZE8BYTE,gu8CSASsendsasoutput3);


     }
	 else if (((can_msg[0] & 0x0F)==0x0F) && (can_msg[1] == 0xFF)  && (can_msg[2] == 0x07)
			 && (can_msg[3] == 0x01)&& (can_msg[4] == 0x00)&& (can_msg[5] == 0x00)&& (can_msg[6] == 0x6D))
	 {
		...
		gu16FinalSasAngle=0;
		gu8CSASsendsasoutput3[2]=(byte0_msb<<4|0x0F);
		gu8CSASsendsasoutput3[7]=0x3C;
				CAN_Transmit_msg(&gCANMailBoxNo, CSAS525SASARBID_SCAN_TRX, CAN_ID_STD,
						      CSAS525_SIZE8BYTE,gu8CSASsendsasoutput3);
		}
	 }
	 ...
	 else if(((can_msg[0])==0x00)   && (can_msg[2] == 0x07)  && ((can_msg[6] & 0x0F)==0x2F))
		 {
		 CAN_Transmit_msg(&gCANMailBoxNo, CSAS525SASARBID_SCAN_TRX, CAN_ID_STD,
								      CSAS525_SIZE8BYTE,gu8CSASsendsasoutput3);
	  }
	 else if((can_msg[0] == 0x70) && (can_msg[1] == 0x04)  && (can_msg[2] == 0x07) && (can_msg[3] == 0x01)  && (can_msg[6] == 0xA3))
	 {

			byte0_msb=((can_msg[0] & 0xF0)>>4);
			byte1_msb=((can_msg[3] & 0xF0)>>4);
			if((byte0_msb % 2) && (byte1_msb % 2))
			{
				gu16FinalSasAngle=0;
				...
				write_cal_data[1]=(uint8_t)gu16ZeroPositionS1;;
				write_cal_data[2]=(uint8_t)(gu16ZeroPositionS1>>8);
				write_cal_data[3]=(uint8_t)(gu16ZeroPositionS2);
				write_cal_data[4]=(uint8_t)(gu16ZeroPositionS2>>8);
				Cal_Stop=0;
				CalOk=1;
				gu8CSASCanDataFrame[0] =0x00;
				gu8CSASCanDataFrame[1]=0x00;
				gu8CSASCanDataFrame[2] =0x10;
				gu8CSASCanDataFrame[3]=can_msg[1];
				gu8CSASCanDataFrame[4] =0xA0;
				gu8CSASCanDataFrame[5]=0x00;
				gu8CSASCanDataFrame[6] =0x00;
				gu8CSASCanDataFrame[7]=0xE1;
